...
userApp.get('/add-count',expressAsyncHandler(async(request,response)=>{
  console.log("hi")
  const userObj1 = request.app.get("userCollection")
  const result=await userObj1.updateOne({typeofuser:"User"},{ $set:  { "count": 0} } );
  response.status(201).send({ message: "Nullified Count" })
}))
