...
function AllAudis() {
  ...
  let getAudis=()=>{
    ...
    let token=localStorage.getItem("token")
     axios.get("/AdminHome-api/get-audi",{headers:{"Authorization":"Bearer "+token}})
     .then((response)=>{
...
let saveAudi=()=>{
  ...
  axios.put(`/AdminHome-api/edit-audi/${modifiedAudi.id}`,modifiedAudi)
  .then(res=>{
    if(res.status===201)
    {
      console.log("done1");
  ...
  let deleteAudi =(Name1) =>{
      ...
      if(window.confirm(`Are you sure you want to delete? ${Name1}`)){
        axios.delete(`/AdminHome-api/remove/${Name1}`).then((response)=>{
          audioElement.play(); 
      Swal.fire({
        icon: 'success',
        title: Name1,
