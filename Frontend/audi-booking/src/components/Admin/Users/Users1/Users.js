...
function Users1() {
  ...
  //get
    ...
    const [lastNullificationTimestamp, setLastNullificationTimestamp] = useState(
      parseInt(localStorage.getItem('lastNullificationTimestamp')) || 0
    );
  
    const addCount = () => {
      const currentTimestamp = Date.now()+28;
      const daysPassed = (currentTimestamp - lastNullificationTimestamp) / (1000 * 60 * 60 * 24);
      console.log(currentTimestamp,daysPassed)
    
      if (daysPassed >= 15) {
        // Add your count increment logic here
        // For example, increment a count state
        // setCount(count + 1);
        
        // Update the last nullification timestamp
        setLastNullificationTimestamp(currentTimestamp);
  
        // Update localStorage
        localStorage.setItem('lastNullificationTimestamp', currentTimestamp.toString());
        if(window.confirm(`Are you sure you want to nullify Count`))
        {
        axios.get("/ConsumerHome-api/add-count")
        .then((response)=>{
          if(response.status===201)
          {
            Swal.fire({
              icon: 'success',
              title: 'Nullified Count Successfully!',
              text: ' Count has been changed successfully.',
            });
          }
        })
      }
      } else {
        // Notify the user that they can only nullify the count after 15 days
        alert('You can only nullify the count after 15 days.');
      }
    };
  ...
  let getUsers1=()=>{
     ...
     axios.get("/ConsumerHome-api/get-user")
     .then((response)=>{
...
let EditUser=(Name1Tobeedited)=>{
  ...
  let saveUser=()=>{
    ...
    axios.put(`/ConsumerHome-api/edit-user/${modifiedUser.id}`,modifiedUser)
    .then(res=>{
      if(res.status===201)
      {
        console.log("done1");
...
let deleteUser =(Name1) =>{
    ...
    if(window.confirm(`Are you sure you want to delete? ${Name1}`)){
      axios.delete(`/ConsumerHome-api/remove/${Name1}`).then((response)=>{
        audioElement.play(); 
    Swal.fire({
      icon: 'success',
      title: Name1,
  ...
  return (
    <div>
          <button onClick={()=>{addCount()}} className='btn2 btn-success'><AddIcon/>Nullify Count</button>
